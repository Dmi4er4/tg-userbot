...
export function getPeerId(message: Api.Message): string | null {
    const p = message.peerId;
    if (p instanceof Api.PeerUser) return String(p.userId);
    if (p instanceof Api.PeerChat) return String(p.chatId);
    if (p instanceof Api.PeerChannel) return String(p.channelId);
    return null;
}
